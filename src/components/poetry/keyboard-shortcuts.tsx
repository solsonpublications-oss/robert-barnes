...
export function KeyboardShortcuts() {
  ...
  // 'B' → open the reading list panel (click the floating trigger)
  const openReadingList = useCallback(() => {
    document
      .querySelector('button[aria-label="Open reading list"]')
  ...
  const jumpToPoem = useCallback(() => {
    document
      .getElementById("poem-of-the-day")
      ?.scrollIntoView({ behavior: "smooth" });
  }, []);

  // 'R' → jump to resources
  const jumpToResources = useCallback(() => {
    document
...
function ShortcutHint() {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => setShow(false), 11000);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
  ...
  return (
    <div className="fixed bottom-6 left-1/2 z-40 flex -translate-x-1/2 flex-wrap items-center justify-center gap-1.5 rounded-full border border-border/60 bg-card/90 px-4 py-2 text-xs text-muted-foreground shadow-xl backdrop-blur animate-[fadeUp_0.5s_ease-out]">
